 ...
 * 
 */
public class FilterRedGreenSwap extends ImageRegionFilter {
			...
			for (int y = 0; y < img.getHeight(); y++)
				for (int x = 0; x < img.getWidth(); x++) {
					...
					if (y >= getMinY() && y <= getMaxY() && x >= getMinX()
						...
						// Compose the new pixel, swapping locations of green
						// and red
						int newPixel = (greenAmount << 16) | (redAmount << 8)
								| blueAmount;
		...
		// If there is no region selected, filter the entire image
			...
			for (int y = 0; y < img.getHeight(); y++)
				for (int x = 0; x < img.getWidth(); x++) {
					...
					// Compose the new pixel, swapping locations of green and
					// red
					int newPixel = (greenAmount << 16) | (redAmount << 8)
							| blueAmount;
